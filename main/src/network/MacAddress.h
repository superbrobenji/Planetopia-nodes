...
struct MacAddress {
  ...
  bool isZero() const { for (auto b: bytes) if (b != 0) return false; return true; }
  bool isBroadcast() const { for (auto b: bytes) if (b != 0xFF) return false; return true; }
...
inline MacAddress::MacAddress(const String& macString) {
  ...
  int vals[6];
  if (sscanf(macString.c_str(), "%02x:%02x:%02x:%02x:%02x:%02x", &vals[0], &vals[1], &vals[2], &vals[3], &vals[4], &vals[5]) == 6) {
    for (int i = 0; i < 6; ++i) bytes[i] = static_cast<uint8_t>(vals[i]);
  } else {
    setZero();
  }
}
